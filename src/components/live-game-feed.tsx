  ...
  return (
    <Card className="p-4 sm:p-6 bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-950 border-2">
      <div className="flex items-center gap-2 mb-4">
        ...
        <Badge
          variant="secondary"
          className="ml-auto bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 text-xs"
        >
        ...
        {activities.map((activity, index) => (
          ...
          >
            <div className="flex-1 min-w-0">
              ...
              <p className={`text-xs sm:text-sm ${getActionColor(activity.action)}`}>{getActionText(activity)}</p>
            </div>
          </div>
        ))}
      </div>
