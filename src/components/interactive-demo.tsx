...
export function InteractiveDemo() {
  ...
  const submitGuess = () => {
    ...
    if (!DEMO_WORDS.includes(currentGuess.toUpperCase())) {
  ...
  const getKeyStyle = (letter: string) => {
    ...
    const baseStyle =
      'px-2 sm:px-3 py-3 sm:py-4 rounded font-bold text-xs sm:text-sm transition-all duration-200 hover:scale-105 min-w-[1.75rem] sm:min-w-[2rem]'
      ...
      <CardContent className="space-y-3 sm:space-y-4 relative">
        {errorMsg && (
          <div className="mb-2 text-center">
            <span className="inline-flex items-center px-3 py-1 rounded-full bg-red-600/90 text-white text-xs sm:text-sm shadow-md">
              {errorMsg}
            </span>
          </div>
        )}
        ...
        {gameOver && (
          <div className="text-center space-y-3 p-4 bg-gradient-to-br from-blue-50 to-purple-50 dark:from-blue-950/20 dark:to-purple-950/20 rounded-lg">
            {won ? (
              <>
                <p className="text-xl font-bold text-green-600 dark:text-green-400">👏 Awesome! You got it!</p>
              ...
              <>
                <p className="text-xl font-bold text-orange-600 dark:text-orange-400">Demo complete!</p>
                <p className="text-sm text-muted-foreground">
                  The word was <span className="font-bold text-foreground">{DEMO_WORD}</span>
                </p>
                <p className="text-xs text-muted-foreground">In the real game, you can win SOL prizes!</p>
              </>
            )}
